...
class SpringBootWebfluxApirestApplicationTests {
	...
	public void editarTest() {
		Producto producto = service.findByNombre("Sony Camara").block();
		Categoria categoria = service.findCategoriaByNombre("Electrónico").block();
		
		Producto productoEditado = new Producto("Panasonic Camara", 700.00, categoria);
		...
		.expectBody()
		.jsonPath("$.id").isNotEmpty()
		.jsonPath("$.nombre").isEqualTo("Panasonic Camara")
		.jsonPath("$.categoria.nombre").isEqualTo("Electrónico");
	}
	
	@Test
	public void eliminarTest() {
		Producto producto = service.findByNombre("Mica comoda 5").block();
		client.delete()
		.uri(url + "/{id}", Collections.singletonMap("id", producto.getId()))
		.exchange()
